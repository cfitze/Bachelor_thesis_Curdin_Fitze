...
def start_dash_plot():
    ...
    # @app.callback(Output('url', 'pathname'), [Input('url', 'pathname')], [State('url', 'pathname')])
    # def go_back_to_main_page(current_pathname, previous_pathname):
    #     if current_pathname == '/subpage' and previous_pathname == '/subpage':
    #         return '/'
    #     else:
    #         return current_pathname
    app.run_server(port=8020,debug=True, dev_tools_ui=True) #dev_tools_hot_reload_interval=1000, dev_tools_silence_routes_logging=False)
    # app.run_server()
