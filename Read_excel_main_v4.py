...
class PlotExcel(MainClass):
    ...
    def start_dash_plot(self, import_excel):
        ...
        # datetime_columt_tolist = datetime_column.tolist(),
        ...
        self.app.layout = html.Div(children=[
            ...
            dcc.RadioItems(
                ...
                value=[{'label': column_inital, 'value': column_inital} for column_inital in columns_available_without_datetime_inital],  # Set the initial value to the first column
        ...
        # Callback to update the plot on the selected columns
        @self.app.callback(
            [dash.dependencies.Output('3d-bar-plot', 'figure'),
            dash.dependencies.Output('selected-date-radio-items', 'options')],
            [dash.dependencies.Input('selected-date-radio-items', 'value'),
            dash.dependencies.Input('store', 'data')]
        )
        def update_datasets(selected_columns, stored_data):
            ...
            start_date = pd.to_datetime(stored_data['initial_first_date'])
            end_date = pd.to_datetime(stored_data['initial_last_date'])

            figure = self.create_plot_figure(initial_data_without_datetime, datetime_column, start_date, end_date, selected_columns)
        ...
        def update_plot(selected_date_range, stored_data):
            datetime_column = pd.to_datetime(stored_data['datetime_column'])
            ...
            initial_data_without_datetime = pd.DataFrame(stored_data['initial_data_without_datetime'])
            selected_columns = dash.callback_context.inputs['selected-date-radio-items.value']
    ...
    def create_plot_figure(self, data_without_datetime, datetime_column ,start_date, end_date, selected_columns):
        ...
        # Convert data from the store back into DataFrame and datetime objects
        # initial_data_without_datetime = pd.DataFrame(initial_data_without_datetime['initial_data_without_datetime'])
        # datetime_column = pd.to_datetime(initial_data_without_datetime['datetime_column'])
